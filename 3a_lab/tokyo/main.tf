...
# cb 0118 4:45
resource "aws_vpc" "shinjuku_011826-vpc01" {
...
# cb 4:50
resource "aws_subnet" "shinjuku_public-subnets0" {
...
resource "aws_nat_gateway" "shinjuku_nat01" {
  ...
  subnet_id     = aws_subnet.shinjuku_public-subnets0[0].id # NAT in a public subnet
...
resource "aws_route_table_association" "shinjuku_public_rta" {
  count          = length(aws_subnet.shinjuku_public-subnets0)
  subnet_id      = aws_subnet.shinjuku_public-subnets0[count.index].id
  route_table_id = aws_route_table.shinjuku_public_rt01.id
}
...
# Explanation: Private subn'ets route outbound internet via NAT (shinjuku-approved stealth).
...
resource "aws_security_group" "shinjuku_ec2_lab" {
  name        = "${local.name_prefix}-ec2-sg01"
  description = "EC2 app security group"
  vpc_id      = aws_vpc.shinjuku_011826-vpc01.id
 
 # 0119 cb 1205 need to change to SSM
# ingress {
...
resource "aws_security_group" "shinjuku_rds_sg01" {
  ...
  ingress {
    ...
    to_port     = 3306
    protocol    = "tcp"
    security_groups = [aws_security_group.shinjuku_ec2_lab.id]
    cidr_blocks       = ["10.162.0.0/16"]
  }
...
# 03082026 Explanation: Tokyo RDS (PHI stored here only)The entire path stays on the AWS backbone and is encrypted in transit.RDS hides in private subnets like the Rebel base on Hoth—cold, quiet, and not public.
...
# 03082026 Explanation: Tokyo RDS (PHI stored here only)The entire path stays on the AWS backbone and is encrypted in transit.This is the holocron of state—your relational data lives here, not on the EC2.
...
resource "aws_iam_role" "shinjuku-ec2-role01" {
...
resource "aws_iam_role_policy_attachment" "shinjuku_ec2_ssm_attach" {
  role       = aws_iam_role.shinjuku-ec2-role01.name
...
# 0119 1200 commented out to give way for the new ec2 on bonus_a.tf line 22
# resource "aws_instance" "shinjuku_ec201" {
#   ami                    = var.ec2_ami_id
#   instance_type           = var.ec2_instance_type
#   subnet_id               = aws_subnet.shinjuku_public-subnets0[0].id
#   vpc_security_group_ids  = [aws_security_group.shinjuku_ec2_lab.id]
...
# 030826 Explanation: Tokyo Parameter Store The entire path stays on the AWS backbone and is encrypted in transit.Parameter Store is shinjuku’s map—endpoints and config live here for fast recovery.
# 0118 
resource "aws_ssm_parameter" "shinjuku_db_endpoint_param" {
  name  = "/lab/db/endpoint"
  type  = "String"
  value = "aws_db_instance.shinjuku_rds01.address" # TODO: replace with aws_db_instance.shinjuku_rds01.address once RDS is uncommented
...
# 03082026 cb Explanation: Ports are boring, but even Wookiees need to know which door number to kick in.
# 0308
resource "aws_ssm_parameter" "shinjuku_db_port_param" {
  name  = "/lab/db/port"
  type  = "String"
  value = tostring(aws_db_instance.shinjuku_rds01.port) # TODO: replace with tostring(aws_db_instance.shinjuku_rds01.port) once RDS is uncommented
...
# 03082026 Explanation: DB name is the label on the crate—without it, you’re rummaging in the dark.
...
# 030826 Explanation: Tokyo Secerts MangerExplanation: Secrets Manager is shinjuku’s locked holster—credentials go here, not in code.
# 0118 7:53
...
# 03082026 Explanation: Secret payload—students should align this structure with their app (and support rotation later).
# 0118 7:33
resource "aws_secretsmanager_secret_version" "shinjuku_db_secret_version01" {
  ...
  secret_string = jsonencode({
    username = var.db_username
    password = var.db_password
    host     = aws_db_instance.shinjuku_rds01.address # TODO: replace with aws_db_instance.shinjuku_rds01.address once RDS is uncommented
    port     = aws_db_instance.shinjuku_rds01.port     # TODO: replace with aws_db_instance.shinjuku_rds01.port once RDS is uncommented
    dbname   = var.db_name
  })
}
...
# 030826 Explanation: Tokyo CW Explanation: When the Falcon is on fire, logs tell you *which* wire sparked—ship them centrally.
# 0118 7:30
...
# 030826 Explanation: Tokyo CW Explanation: Metrics are shinjuku’s growls—when they spike, something is wrong.
...
# 030826 Explanation: Tokyo SNS Pager Duty Explanation: SNS is the distress beacon—when the DB dies, the galaxy (your inbox) must hear about it.
#5:24 0118
resource "aws_sns_topic" "shinjuku_sns_topic01" {
  name = "${local.name_prefix}-db-incidents"
}

# 030826 Explanation: Tokyo SNS Pager Duty Explanation: Email subscription = “poor man’s PagerDuty”—still enough to wake you up at 3AM.
# 5:24 0118
