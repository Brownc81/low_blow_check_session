...
# Explanation: The zone apex is the throne room—liberdade-growl.com itself should lead to the ALB.
resource "aws_route53_record" "liberdade_apex_alias01" {
  zone_id = var.route53_hosted_zone_id #local.liberdade_zone_id
  name    = var.domain_name
  type    = "A"
  allow_overwrite = true # this was add 03/22/2026 just migt fix everthing
...
# # NOTE: This is a skeleton patch: students must merge this into aws_lb.liberdade_alb01
# # by adding/accessing the `access_logs` block. Terraform does not support "partial" blocks.
# #
# # Add this inside resource "aws_lb" "liberdade_alb01" { ... } in bonus_b.tf:
# #
# # access_logs {
