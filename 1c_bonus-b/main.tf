...
# cb 0118 4:45
resource "aws_vpc" "armageddon_011826-vpc01" {
...
resource "aws_internet_gateway" "armageddon_igw01" {
  vpc_id = aws_vpc.armageddon_011826-vpc01.id
...
# cb 4:50
resource "aws_subnet" "armageddon_public-subnets0" {
...
resource "aws_nat_gateway" "armageddon_nat01" {
  ...
  subnet_id     = aws_subnet.armageddon_public-subnets0[0].id # NAT in a public subnet
...
resource "aws_route_table_association" "armageddon_public_rta" {
  count          = length(aws_subnet.armageddon_public-subnets0)
  subnet_id      = aws_subnet.armageddon_public-subnets0[count.index].id
  route_table_id = aws_route_table.armageddon_public_rt01.id
}
...
# Explanation: Private subn'ets route outbound internet via NAT (armageddon-approved stealth).
...
resource "aws_security_group" "arrmageddon_ec2_lab" {
...
#   subnet_id               = aws_subnet.armageddon_public-subnets0[0].id
...
resource "aws_secretsmanager_secret" "armageddon_db_secret01" {
  name = "${local.name_prefix}/lab/rds/sql"
