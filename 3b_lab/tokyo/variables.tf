...
variable "project_name" {
  description = "Prefix for naming. Students should change from 'shinjuku' to their own."
  type        = string
  default     = "shinjuku"
}
...
variable "azs" {
  ...
  type        = list(string)
  default     = ["ap-northeast-1a", "ap-northeast-1d"] # TODO: student supplies 0118 1010
}

variable "ec2_ami_id" {
  description = "AMI ID for the EC2 app host."
  type        = string
  default     = "ami-088b486f20fab3f0e" # TODO 032126
}
...
variable "cloudfront_acm_cert_arn" {
  ...
  type        = string
  default = "arn:aws:acm:us-east-1:400398151894:certificate/5d8a60ad-699c-4119-8bcc-4863bb41971f"
}  
# ── Lab 3A TGW variables ──────────────────────────────────────────────────────
variable "saopaulo_vpc_cidr" {
  description = "São Paulo VPC CIDR — used for RDS SG rule and TGW return route."
  type        = string
  default     = "10.162.0.0/16" # Must be different from Tokyo's 10.161.0.0/16
}

variable "saopaulo_tgw_id" {
  description = "São Paulo TGW ID — copy from São Paulo outputs after first apply."
  type        = string
  default     = "tgw-0654079992440ab34" # TODO: 03/28/2026 paste liberdade_tgw_id output from São Paulo after apply
}
