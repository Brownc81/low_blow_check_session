...
resource "aws_security_group_rule" "shinjuku_ec2_ingress_from_alb01" {
  type                     = "ingress"
  security_group_id        = aws_security_group.shinjuku_ec2_lab.id
  from_port                = 80
  to_port                  = 80
  protocol                 = "tcp"
  source_security_group_id = aws_security_group.shinjuku_alb_sg01.id
...
resource "aws_lb" "shinjuku_alb01" {
  ...
  subnets         = aws_subnet.shinjuku_public-subnets0[*].id
...
# # ############################################
# # # ACM Certificate (TLS) for app.shinjuku-growl.com
# # ############################################
...
# # TODO: students implement aws_route53_record(s) if they manage DNS in Route53.
 resource "aws_route53_record" "shinjuku_acm_validation" {
  ...
  zone_id = var.route53_hosted_zone_id #local.shinjuku_zone_id
...
resource "aws_acm_certificate_validation" "shinjuku_acm_validation01" {
  ...
  validation_record_fqdns = [for r in aws_route53_record.shinjuku_acm_validation : r.fqdn]
}
