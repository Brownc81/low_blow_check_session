...
locals {
 ...
 liberdade_fqdn = "${var.app_subdomain}.${var.domain_name}"
}
...
resource "aws_security_group" "liberdade_alb_sg01" {
  ...
  # 0123 cb 0158 
  # 021426 Now add/replace your ALB SG ingress:
# ingress {
...
#   }
 ...
 egress {
    description = "Allow outbound to private network"
    from_port   = 0
    to_port     = 0
    protocol    = "-1"
    cidr_blocks = ["0.0.0.0/0"]
...
resource "aws_security_group_rule" "liberdade_ec2_ingress_from_alb01" {
  type                     = "ingress"
  security_group_id        = aws_security_group.liberdade_ec2_lab.id
  provider = aws.saopaulo
  from_port                = 80
  to_port                  = 80
  protocol                 = "tcp"
  source_security_group_id = aws_security_group.liberdade_alb_sg01.id
...
resource "aws_lb" "liberdade_alb01" {
  ...
  subnets         = aws_subnet.liberdade_public-subnets0[*].id
...
# # ############################################
# # # ACM Certificate (TLS) for app.liberdade-growl.com
# # ############################################
...
resource "aws_route53_record" "liberdade_acm_validation" {
   ...
   provider = aws.east # Ensure Route 53 actions happen in the global region
...
resource "aws_acm_certificate_validation" "liberdade_acm_validation01" {
  ...
  validation_record_fqdns = [for r in aws_route53_record.liberdade_acm_validation : r.fqdn]
}
