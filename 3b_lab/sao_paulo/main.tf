...
# cb 0118 4:45
resource "aws_vpc" "liberdade_011826-vpc01" {
...
# cb 4:50
resource "aws_subnet" "liberdade_public-subnets0" {
...
resource "aws_nat_gateway" "liberdade_nat01" {
  ...
  subnet_id     = aws_subnet.liberdade_public-subnets0[0].id # NAT in a public subnet
  provider = aws.saopaulo
  tags = {
  ...
  depends_on = [
    aws_internet_gateway.liberdade_igw01,
    aws_eip.liberdade_nat_eip01,
  ]
}
...
resource "aws_route_table_association" "liberdade_public_rta" {
  count          = length(aws_subnet.liberdade_public-subnets0)
  provider = aws.saopaulo
  subnet_id      = aws_subnet.liberdade_public-subnets0[count.index].id
  route_table_id = aws_route_table.liberdade_public_rt01.id
}
...
# Explanation: Private subn'ets route outbound internet via NAT (liberdade-approved stealth).
...
resource "aws_security_group" "liberdade_ec2_lab" {
...
# # *********03/??/2026 need to allow this SG to accept traffic for tokyo cidir range form sao********
...
#another ingress rule for sao paulo data look up, or manually input
# ingress {
#     description = "???"
#     from_port   = ????
#     to_port     = ????
#     protocol    = "tcp"
#     security_groups = [aws_security_group.arrmageddon_ec2_lab.id]
#   }
...
# 03082026 It does not contain: Explanation: RDS hides in private subnets like the Rebel base on Hoth—cold, quiet, and not public.
...
resource "aws_iam_role" "liberdad-ec2-role01" {
...
resource "aws_iam_role_policy_attachment" "liberdade_ec2_ssm_attach" {
  role       = aws_iam_role.liberdad-ec2-role01.name
...
#   subnet_id               = aws_subnet.liberdade_public-subnets0[0].id
#   vpc_security_group_ids  = [aws_security_group.arrmageddon_ec2_lab.id]
