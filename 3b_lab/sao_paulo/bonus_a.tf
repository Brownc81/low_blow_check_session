...
locals {
  liberdade_prefix           = var.project_name
  liberdade_secret_arn_guess = "arn:aws:secretsmanager:${data.aws_region.liberdade_region01.id}:${data.aws_caller_identity.liberdade_self01.account_id}:secret:${local.liberdade_prefix}/rds/mysql*"
}
...
resource "aws_instance" "liberdade_ec201_private_bonus" {
  ...
  provider = aws.saopaulo
  vpc_security_group_ids = [aws_security_group.liberdade_ec2_lab.id]
  ...
  # 0119 cb 1205 need to change to  (session manager allows the ec2 to communicate with out an ingress rule) commented out 80 and 22
...
ingress {
    ...
    to_port     = 443
    protocol    = "tcp"
    security_groups = [aws_security_group.liberdade_ec2_lab.id]
  }
...
resource "aws_vpc_endpoint" "liberdade_vpce_s3_gw01" {
  vpc_id            = aws_vpc.liberdade_011826-vpc01.id
  service_name      = "com.amazonaws.${data.aws_region.liberdade_region01.id}.s3"
...
resource "aws_vpc_endpoint" "liberdade_vpce_ssm01" {
  vpc_id              = aws_vpc.liberdade_011826-vpc01.id
  service_name        = "com.amazonaws.${data.aws_region.liberdade_region01.id}.ssm"
...
resource "aws_vpc_endpoint" "liberdade_vpce_ec2messages01" {
  vpc_id              = aws_vpc.liberdade_011826-vpc01.id
  service_name        = "com.amazonaws.${data.aws_region.liberdade_region01.id}.ec2messages"
  vpc_endpoint_type   = "Interface"
  private_dns_enabled = true
  provider = aws.saopaulo
  subnet_ids         = aws_subnet.liberdade_private_subnets[*].id
...
resource "aws_vpc_endpoint" "liberdade_vpce_ssmmessages01" {
  vpc_id              = aws_vpc.liberdade_011826-vpc01.id
  service_name        = "com.amazonaws.${data.aws_region.liberdade_region01.id}.ssmmessages"
  vpc_endpoint_type   = "Interface"
  private_dns_enabled = true
  provider = aws.saopaulo
  subnet_ids         = aws_subnet.liberdade_private_subnets[*].id
...
resource "aws_vpc_endpoint" "liberdade_vpce_logs01" {
  vpc_id              = aws_vpc.liberdade_011826-vpc01.id
  service_name        = "com.amazonaws.${data.aws_region.liberdade_region01.id}.logs"
  vpc_endpoint_type   = "Interface"
  private_dns_enabled = true
  provider = aws.saopaulo
  subnet_ids         = aws_subnet.liberdade_private_subnets[*].id
  ...
  tags = {
    Name = "${local.liberdade_prefix}-vpce-logs01"
  }
}
...
resource "aws_vpc_endpoint" "liberdade_vpce_secrets01" {
  vpc_id              = aws_vpc.liberdade_011826-vpc01.id
  service_name        = "com.amazonaws.${data.aws_region.liberdade_region01.id}.secretsmanager"
  vpc_endpoint_type   = "Interface"
  private_dns_enabled = true
  provider = aws.saopaulo
  subnet_ids         = aws_subnet.liberdade_private_subnets[*].id
  ...
  tags = {
    Name = "${local.liberdade_prefix}-vpce-secrets01"
  }
}
...
resource "aws_vpc_endpoint" "liberdade_vpce_kms01" {
  vpc_id              = aws_vpc.liberdade_011826-vpc01.id
  service_name        = "com.amazonaws.${data.aws_region.liberdade_region01.id}.kms"
  vpc_endpoint_type   = "Interface"
  private_dns_enabled = true
  provider = aws.saopaulo
  subnet_ids         = aws_subnet.liberdade_private_subnets[*].id
  ...
  tags = {
    Name = "${local.liberdade_prefix}-vpce-kms01"
  }
}
...
#         ]
#         Resource = [
#           "arn:aws:ssm:${data.aws_region.liberdade_region01.id}:${data.aws_caller_identity.liberdade_self01.account_id}:parameter/lab/db/*"
#         ]
#       }
#     ]
#   })
...
#   role       = aws_iam_role.liberdade-ec2-role01.id
