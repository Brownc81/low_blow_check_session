...
resource "aws_security_group" "chewbacca_alb_sg01" {
  ...
  # 0123 cb 0158 
  # 020426 Now add/replace your ALB SG ingress:
  # ingress {
...
resource "aws_security_group_rule" "chewbacca_ec2_ingress_from_alb01" {
  type                     = "ingress"
  security_group_id        = aws_security_group.arrmageddon_ec2_lab.id
  from_port                = 80
  to_port                  = 80
  protocol                 = "tcp"
...
resource "aws_lb" "chewbacca_alb01" {
  ...
  subnets         = aws_subnet.armageddon_public-subnets0[*].id
...
resource "aws_route53_record" "armageddon_acm_validation" {
  for_each = var.certificate_validation_method == "var.domain_name" ? {
  ...
  zone_id         = "Z0512201FXFQUJ6HG6NP" #local.chewbacca_zone_id
...
resource "aws_acm_certificate_validation" "chewbacca_acm_validation01" {
  ...
  validation_record_fqdns = [for r in aws_route53_record.armageddon_acm_validation : r.fqdn]
}
